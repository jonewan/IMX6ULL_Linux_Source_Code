...
void lcd_drawline(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
	...
	for(t = 0; t <= distance+1; t++ ){	/* 画线输出 */
		lcd_draw_point(uRow, uCol, tftlcd_dev.fore_color);/* 画点 */
		xerr += delta_x ;
		yerr += delta_y ;
		if(xerr > distance){
			xerr -= distance;
...
void lcd_draw_Circle(uint16_t x0,uint16_t y0,uint8_t r)
{
	...
	while(y > x){   	/* y>x即第一象限的第1区八分圆 */
		lcd_draw_point(x  + mx, y  + my, tftlcd_dev.fore_color);
		lcd_draw_point(y  + mx, x  + my, tftlcd_dev.fore_color);
		lcd_draw_point(-x + mx, y  + my, tftlcd_dev.fore_color);
		lcd_draw_point(-y + mx, x  + my, tftlcd_dev.fore_color);
		lcd_draw_point(-x + mx, -y + my, tftlcd_dev.fore_color);
		lcd_draw_point(-y + mx, -x + my, tftlcd_dev.fore_color);
				      ...
				      uint8_t mode)
{
	...
	for(t = 0; t < csize; t++)
	{
		...
		for(t1 = 0; t1 < 8; t1++){
			if(temp & 0x80)lcd_draw_point(x, y, tftlcd_dev.fore_color);
			else if(mode==0)lcd_draw_point(x, y, tftlcd_dev.back_color);
			temp <<= 1;
			y++;
